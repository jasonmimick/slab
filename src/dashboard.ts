...
<style>
  ...
  * { box-sizing: border-box; margin: 0; }
  ...
  /* direct app links for a peer node — tokenless, via this node's ingress */
  .applinks { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 12px; padding-top: 12px;
    border-top: 1px dashed var(--edge2); }
  .applink { display: inline-flex; align-items: center; gap: 6px; font-size: 11px; text-decoration: none;
    color: var(--ink); background: var(--unit1); border: 1px solid var(--edge2); border-radius: 6px;
    padding: 3px 9px 3px 7px; letter-spacing: .01em; transition: border-color .12s, transform .12s; }
  .applink:hover { border-color: var(--accent); transform: translateY(-1px); }
  .applink .od { width: 7px; height: 7px; }
  .otile {
...
// A peer's rack UI lives on its own origin (jasons-mac-mini:7766) with its own
// token in localStorage — never seeded on this laptop. Carry the token in the
// URL once; the dashboard bootstrap stashes it and strips the query string.
function rackUrl(node) {
  if (!node.url) return ''
  return node.url + (node.token ? '/?token=' + encodeURIComponent(node.token) : '')
}
// A peer app opens through THIS node's ingress: <app>.<peer>.localhost:<proxy>
// reverse-proxies to the peer's ingress with no token needed. So from the train,
// clicking a mac-mini app just works — one rack you can address.
function appUrl(node, appName) {
  return node.self
    ? 'http://' + appName + '.localhost:${proxyPort}/'
    : 'http://' + appName + '.' + node.name + '.localhost:${proxyPort}/'
}
function nodeBand(node) {
  ...
  const openRemote = node.url ? "window.open('" + esc(rackUrl(node)) + "')" : ''
  ...
  // Direct, tokenless links to each public app on a peer — the useful bit from a
  // remote node. Private apps have no ingress, so they aren't linkable.
  const links = node.self ? [] : node.apps
    .filter(a => a.manifest.public !== false)
    .sort((a, b) => a.name.localeCompare(b.name))
    .map(a => '<a class="applink ' + a.state + '" href="' + esc(appUrl(node, a.name)) + '" target="_blank" title="open ' + esc(a.name) + ' on ' + esc(node.name) + '"><i class="od ' + a.state + '"></i>' + esc(a.name) + '</a>')
  return '<div class="nodeband' + (node.self ? ' here' : '') + (node.reachable ? '' : ' down') + '">'
    ...
    + (node.self ? '<span class="heretag">this node</span>'
        : node.url ? '<a href="' + esc(rackUrl(node)) + '" target="_blank">open rack &#8599;</a>' : '')
    + (node.reachable ? '' : '<span class="nerr">unreachable</span>')
    + '<span class="nstats">' + running + '/' + node.apps.length + ' up · ' + rpm + ' req/m</span>'
    + '</div><div class="ovgrid">' + tiles + '</div>'
    + (links.length ? '<div class="applinks">' + links.join('') + '</div>' : '')
    + '</div>'
}
